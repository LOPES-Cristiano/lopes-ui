...
const ALL_GROUPS: ToolbarGroup[] = [
  "history", "block", "inline", "align", "list", "link", "table", "media",
];
  ...
  return (
    <button
      ...
      className={twMerge(
        ...
        active && "bg-zinc-100 dark:bg-zinc-700 text-zinc-800 dark:text-zinc-100",
      )}
    >
...
function Sep() {
  return <div className="mx-0.5 h-5 w-px bg-zinc-200 dark:bg-zinc-700 shrink-0" />;
}
...
function Toolbar({
  ...
  return (
    <div className="relative flex flex-wrap items-center gap-0.5 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/60 px-2 py-1.5">
      {/* History */}
      {has("history") && (
        <>
      ...
      {has("link") && (
        <>
          ...
          {editor.isActive("link") && (
            <ToolBtn icon={Link2Off} label="Remover link" onClick={() => editor.chain().focus().unsetLink().run()} />
          )}
      ...
      {/* Media */}
      {has("media") && (
        <ToolBtn icon={ImageIcon} label="Inserir imagem" onClick={insertImage} />
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <EditorContent
        editor={editor}
        className={twMerge(
          "rte-content flex-1 overflow-y-auto px-4 py-3",
      ...
      {showCount && editor && (
        <div className="flex items-center justify-end gap-3 border-t border-zinc-100 dark:border-zinc-800 px-3 py-1.5">
          ...
          <span className={twMerge("text-[11px]", atLimit ? "text-red-500 font-medium" : "text-zinc-400 dark:text-zinc-500")}>
            {charCount}{maxLength !== undefined ? ` / ${maxLength}` : ""} caracteres
          </span>
        </div>
      )}
    </div>
