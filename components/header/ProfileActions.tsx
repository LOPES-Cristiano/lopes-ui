  ...
  return (
    <div className="flex items-center gap-3 px-3 py-3 border-b border-zinc-100">
      ...
      <div className="min-w-0">
        {name && <p className="text-sm font-semibold text-zinc-900 truncate">{name}</p>}
  ...
  const inner = (
    <>
      ...
      <span className="flex-1 text-left min-w-0">
        ...
        {description && (
          <span className="block text-xs text-zinc-400 leading-snug mt-0.5">{description}</span>
        )}
      </span>
      {badge && <span className="shrink-0">{badge}</span>}
      {shortcut && (
        <kbd className="shrink-0 text-[10px] font-mono text-zinc-400 bg-zinc-100 rounded px-1.5 py-0.5">
          {shortcut}
        </kbd>
      )}
      {arrow && <ChevronRight size={13} className="shrink-0 text-zinc-300" />}
    </>
  );
